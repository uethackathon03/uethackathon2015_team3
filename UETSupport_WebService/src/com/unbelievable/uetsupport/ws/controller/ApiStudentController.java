...
import com.unbelievable.uetsupport.models.Reminder;
import com.unbelievable.uetsupport.models.Schedule;
import com.unbelievable.uetsupport.models.Student;
import com.unbelievable.uetsupport.respsec.TokenInfo;
import com.unbelievable.uetsupport.respsec.TokenManager;
import com.unbelievable.uetsupport.ws.response.ResponseDataLogin;
...
public class ApiStudentController extends BaseController {
	...
	public @ResponseBody ResponseObjectDetail<Object> updateProfile(HttpSession httpSession,
		...
		if (CommonUtils.stringIsValid(course)) student.course = course;
		...
		if (avatar != null) {
			if (student.avatar == null) {
				...
				try {
					byte[] bytes = avatar.getBytes();
					File serverFile = new File(dir, FilenameUtils.getName(student.avatar));
					BufferedOutputStream stream = new BufferedOutputStream(new FileOutputStream(serverFile));
	                stream.write(bytes);
	                stream.close();
				} catch (Exception e) {
